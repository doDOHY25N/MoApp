...
public class MainCalendarActinty extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        ImageView goalImg = (ImageView) findViewById(R.id.goal);
        goalImg.setOnClickListener(new View.OnClickListener(){
            public void onClick(View v){
                Intent intents = new Intent(MainCalendarActinty.this, GoalActivity.class);
        ...
        calendarView.setOnDateChangeListener(new CalendarView.OnDateChangeListener() {
            ...
            public void onSelectedDayChange(@NonNull CalendarView calendarView, int year, int month, int day) {
                ...
                Log.d("날짜", date);
                FirebaseUser firebaseUser = firebaseAuth.getCurrentUser(); //현재 로그인 된 유저의 정보
                databaseReference.child(firebaseUser.getUid()).child(String.valueOf(year)).child(String.valueOf(month + 1)).child(String.valueOf(day)).addListenerForSingleValueEvent(new ValueEventListener() {
                    @Override
                    public void onDataChange(@NonNull DataSnapshot snapshot) { //firebase의 데이터를 가져온다.
                        Schedules.clear();
